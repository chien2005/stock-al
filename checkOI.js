/**
 * 🛠️ SCRIPT KIỂM TRA DỮ LIỆU OPEN INTEREST (OI) - VỊ THẾ MỞ CÒN TỒN ĐỌNG QUA ĐÊM
...
async function main() {
  console.log('\n🔍 ĐANG TÍNH TOÁN DỮ LIỆU OPEN INTEREST (OI) & PHÁI SINH...\n');
  try {
    ...
    if (data.realtimeOI && data.realtimeOI.totalOI !== null) {
      ...
      if (data.realtimeOI.estimatedLong !== null) {
        console.log(`   • Vị thế mở: Long ~${data.realtimeOI.estimatedLong.toLocaleString('vi-VN')} HĐ | Short ~${data.realtimeOI.estimatedShort.toLocaleString('vi-VN')} HĐ`);
      }
    } else if (latestOI) {
    ...
    if (data.intradayLS) {
      console.log(`\n📊 KHỚP LỆNH PHIÊN (${data.intradayLS.date}): Mua ${data.intradayLS.longVolume.toLocaleString('vi-VN')} HĐ | Bán ${data.intradayLS.shortVolume.toLocaleString('vi-VN')} HĐ (Tổng ${data.intradayLS.totalVolume.toLocaleString('vi-VN')} HĐ)`);
    }
